  ...
  if ($dir_ok == true) {
      ...
      for ($i=0, $n=sizeof($contents); $i<$n; $i++) {
          ...
          if (isset($buInfo) && is_object($buInfo) && ($entry == $buInfo->file)) {
              ...
              echo '<a href="' . tep_href_link(FILENAME_BACKUP, 'file=' . $entry) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
          } ?>&nbsp;</td>
              </tr>
<?php
      }
...
?>
    ...
    echo tep_draw_button(IMAGE_BACKUP, 'copy', tep_href_link(FILENAME_BACKUP, 'action=backup'));
} if (($action != 'restorelocal') && isset($dir)) {
    echo tep_draw_button(IMAGE_RESTORE, 'arrowrefresh-1-w', tep_href_link(FILENAME_BACKUP, 'action=restorelocal'));
} ?></td>
              </tr>
<?php
