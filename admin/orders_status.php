...
<?php
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
      ...
      if (DEFAULT_ORDERS_STATUS_ID == $orders_status['orders_status_id']) {
          ...
          echo '<a href="' . tep_href_link(FILENAME_ORDERS_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $orders_status['orders_status_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
      } ?>&nbsp;</td>
              </tr>
<?php
  }
...
<?php
  ...
  switch ($action) {
    ...
    case 'delete':
      ...
      if ($remove_status) {
          $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_ORDERS_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->orders_status_id)));
      }
      break;
    default:
