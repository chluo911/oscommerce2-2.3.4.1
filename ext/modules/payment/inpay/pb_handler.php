...
if ($ok) {
    ...
    if ((($invoice_status == "pending")||($invoice_status == "created"))&&(($HTTP_POST_VARS["invoice_status"] == "pending")||($HTTP_POST_VARS["invoice_status"] == "created"))) {
        $ok = true;
    } elseif (($invoice_status == "approved") && ($HTTP_POST_VARS["invoice_status"] == "approved")) {
        $ok = true;
    } elseif (($invoice_status == "sum_too_low") && ($HTTP_POST_VARS["invoice_status"] == "sum_too_low")) {
        $ok = true;
    }
    if (!$ok) {
...
if ($result == 'VERIFIED') {
    ...
    if ($invoice_approved) {
        ...
        switch ($content_type) {
            case 'virtual':
                if ($content_count != sizeof($order->products)) {
                    $content_type = 'mixed';
                }
                break;
            default:
